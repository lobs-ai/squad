...
const TIPS: string[] = [
  ...
  "/tasks shows the current session's task list — agents mark tasks complete as they go.",
  "/questions lists open ask-user prompts the agent is waiting on.",
  ...
  // --- Ask-user answers ---
  "When the agent asks a multi-choice question, type 1/2/3 to pick — or type freeform for \"Other\".",
  ...
  "squad logs -f tails gateway.log — follow what the agent is doing under the hood.",
  "squad key wizard generates an SSH key under docker/data/ssh so agents can git push.",
  ...
  "Type a new message while the agent is working to interrupt it — or wait for the turn to finish.",
  "Ctrl+C during a running agent turn sends a cancel. Press twice in 2s to exit the REPL.",
  "The gateway logs every tool call to SQLite — re-run squad logs to audit what the agent did.",
  ...
  "Kill a stuck agent with /stop — no need to Ctrl+C the whole gateway.",
];
